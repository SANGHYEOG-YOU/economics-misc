...
*   **비교 (탱크 vs 스포츠카):**
    *   **VFI (탱크):** 속도는 느리지만 어떤 험난한 지형(나쁜 초기값, 복잡한 모델)에서도 반드시 목적지에 도착합니다.
    *   **오일러 (스포츠카):** 속도는 압도적이지만, 길이 매끄럽지 않거나 방향을 잘못 잡으면(초기값이 정답에서 멀면) 수렴하지 못하고 엔진이 꺼져버립니다.

2. 비매끄러운(Non-smooth) 문제와 제약 조건 처리
